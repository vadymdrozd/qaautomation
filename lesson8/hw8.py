...
def tickets_check(age: int) -> str:
    ...
    """
    phrases_collection = [
        f'Незважаючи на те, що вам {years_typer(age)}, білетів все одно нема!',
        f'Іди погуляй і приходь пізніше. Може хтось здасть білет для тих, кому {years_typer(age)}',
        f'{years_typer(age)} - це непогано, але для таких як ти в нас білетів немає',
        f'Тобі {years_typer(age)}? Гарно виглядаєш на свій вік ;)',
        f'Тобі {years_typer(age)}? Краще піди подихай свіжим повітрям ;)',
    ]
    str_age = str(age)
    ...
    if age >= 65:
        ...
        return choice(phrases_collection)
